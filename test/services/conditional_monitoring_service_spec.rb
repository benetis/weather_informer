...
class ConditionalMonitoringServiceTest < ActiveSupport::TestCase
  ...
  test '#find_triggers when hot' do
    ...
    assert_equal [{ :hot => hot_forecast.forecast_timestamp }], @service.find_triggers([hot_forecast, not_hot_forecast], [:hot])
  end

  test '#find_triggers when scorching' do
    scorching_forecast = forecasts(:scorching)
    not_scorching_forecast = forecasts(:not_scorching)
    assert_equal [{ :scorching => scorching_forecast.forecast_timestamp }], @service.find_triggers([scorching_forecast, not_scorching_forecast], [:scorching])
  end

  test '#find_triggers when windy' do
    windy_forecast = forecasts(:windy)
    not_windy_forecast = forecasts(:not_windy)
    assert_equal [{ :windy => windy_forecast.forecast_timestamp }], @service.find_triggers([windy_forecast, not_windy_forecast], [:windy])
  end
